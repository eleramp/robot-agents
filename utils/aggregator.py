...
def extract(dpath, subpath):
    ...
    steps_per_key = [min(all_steps, key=len) for all_steps in all_steps_per_key]

    # Get and average wall times / values per step per key
    wall_times_per_key = []
    values_per_key = []
    for k, all_scalar_events in enumerate(all_scalar_events_per_key):
        wall_times = [tuple(scalar_event.wall_time for scalar_event in scalar_events) for scalar_events in all_scalar_events]
        values = [[scalar_event.value for scalar_event in scalar_events] for scalar_events in all_scalar_events]

        for i, el in enumerate(wall_times):
            wall_times[i] = el[:len(steps_per_key[k])]

        for i, el in enumerate(values):
            values[i] = el[:len(steps_per_key[k])]

        wall_times_per_key.append(np.mean(wall_times, axis=0))
