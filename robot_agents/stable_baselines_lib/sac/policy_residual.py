...
def mlp(input_ph, layers, activ_fn=tf.nn.relu, layer_norm=False):
    ...
    :param last_zero: (bool) Wheter to initialize the last layer to be zero or not (used for residual learning)
    :return: (tf.Tensor)
    """
    output = input_ph
    for i, layer_size in enumerate(layers):
        output = tf.layers.dense(output, layer_size, name='fc' + str(i),
                                     kernel_initializer=tf.contrib.layers.xavier_initializer(seed=i))
        if layer_norm:
